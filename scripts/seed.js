...
async function seed() {
    ...
    // --- Admin User ---
    const userConn = await mongoose_1.default.createConnection(`${MONGO_URI}/user-service?authSource=admin`).asPromise();
    ...
    // --- Products ---
    const productConn = await mongoose_1.default.createConnection(`${MONGO_URI}/product-service?authSource=admin`).asPromise();
